# Product Roadmap — Agora

## Milestone 1: Core Foundation & Scaffolding (In Progress)
- [x] Establish repository and `.devpartner/` system tracking
- [x] Formulate high-level Product Definition (README.md)
- [ ] Scaffold Vite + React + TypeScript + Tailwind CSS application workspace

## Milestone 2: Interactive Discovery Prototype
- [ ] Implement Unified Category Navigation Dashboard
- [ ] Build interactive Map exploration UI (interactive mockup SVG/interactive panels)
- [ ] Create detailed cards and modal views for lodging, sports, events, food joints, and essential services

## Milestone 3: Live Events & Community Hub
- [ ] "What's Happening Today" feed/carousel
- [ ] Multi-category quick-filter tags
- [ ] Saved Favorites / Dynamic planner dashboard

## Milestone 4: Polish, Theme & Delivery
- [ ] System-adaptive Light/Dark mode theme toggle
- [ ] Micro-interactions and fluid layout review
- [ ] Final production build compilation check
